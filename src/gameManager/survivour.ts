import { EntityComponentTypes, ItemStack, ItemTypes, Player, world } from "@minecraft/server";


world.afterEvents.itemCompleteUse.subscribe((eventData) => {
    const {itemStack, source} = eventData;
    if(!source.hasTag("survivor")){return;}
...
export const giveSurvivorKit = (player : Player) =>{
