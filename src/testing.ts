...
world.beforeEvents.chatSend.subscribe((eventData) => {
    ...
    if(!cmdPrefixes.some(prefix => message.startsWith(prefix))) { 
        if(isValueInRange(message[0], "a", "z") || isValueInRange(message[0], "A", "Z") || isValueInRange(message[0], "0", "9") ) {return}
...
function up(player, message){ 
    ...
    system.run(() => {
    if(currblock.y+heightNumber > 320){world.sendMessage("Can't place blocks outside the world!"); return}
