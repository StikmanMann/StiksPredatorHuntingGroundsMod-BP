import { system, world, Player, MolangVariableMap, Vector3 } from "@minecraft/server"
...
class DrawFunctions{
    ...
    static drawLine(steps, startLocation, endLocation, particleName,molangVars){
        ...
        for(let i = 0; i < steps; i++){
           ...
           //let test = new MolangVariableMap("variable.color", { red: Math.random(), green: Math.random(), blue: Math.random(), alpha: 1 })
            try{overworld.spawnParticle(particleName, particleLocation, molangVars)}
            catch{}
    ...
    static drawSphere(steps : number, size : number,centerLocation : Vector3, particleName? : string, molangVars?){
       ...
       // world.sendMessage("drawing sphere")
        ...
        for(let i = 0; i < steps; i++){
            ...
            try{overworld.spawnParticle(particleName, particleLocation)}
            catch{}
        }
     ...
     * @param {Player} player 
     */
    static async drawGraph(steps, startLocation, endLocation, player : Player){
            ...
            if(true){
               // Logger.log("SUCCES")
                ...
                for(let i = xOffset; i < newSteps; i++){
                    ...
                    try{ player.applyKnockback(horizontalStrength * -1 , verticalStrength , stepSize * 1.5, (functionY - lastIncrease))}
                    catch{resolve(player)}

                    if(!CollisionFunctions.insideSphere(player.location, particleLocation, 2.25)){
                        try{ player.teleport(particleLocation)}
                        catch(e){Logger.log(e, "FAILED TO TP");resolve(player)}
                        }
