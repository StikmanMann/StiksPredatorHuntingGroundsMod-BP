...
class CollisionFunctions {
    ...
    static insideBox(position, a, b, draw, particleName) {
        ...
        if (draw) {
            DrawFunctions.drawCube(a, b, particleName);
        }
        let isInBox = (position.x > boxStart.x && position.x < boxEnd.x &&
     ...
     * @returns {boolean}
     */
    static insideSphere(position, radiusLocation, radius, draw) {
        let isInSphere = (VectorFunctions.vectorLength(VectorFunctions.subtractVector(position, radiusLocation)) < radius ? true : false);
        if (draw) {
