...
system.run(async () => {
    ...
    world.scoreboard.getObjectives().forEach(scoreboard => {
        ...
        if (scoreboard.displayName.endsWith("Save")) {
            return;
        }
        for (const paricipant of scoreboard.getParticipants()) {
            let foundPlayer = false;
            allPlayers.forEach(player => { /* world.sendMessage(`${paricipant.displayName} == ${player.nameTag}`); */ if (paricipant.displayName == player.nameTag) {
                foundPlayer = true;
            } });
            if (!foundPlayer) { /* world.sendMessage(`Removing: ${paricipant.displayName}`); */
                scoreboard.removeParticipant(paricipant);
            }
        }
    });
});
