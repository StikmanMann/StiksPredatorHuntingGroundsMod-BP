...
//SET POS
{
    ...
    world.afterEvents.playerBreakBlock.subscribe((eventData) => {
        ...
        if (player.hasTag("admin") && player.hasTag("worldEdit")) {
            ...
            try {
                var blocksAffected = (Math.abs(Math.abs(playerInstance.bL1.x) - Math.abs(playerInstance.bL2.x)) + 1) *
                    (Math.abs(Math.abs(playerInstance.bL1.y) - Math.abs(playerInstance.bL2.y)) + 1) *
                    (Math.abs(Math.abs(playerInstance.bL1.z) - Math.abs(playerInstance.bL2.z)) + 1);
            }
            catch {
    ...
    system.run(() => {
        ...
        */
        ...
        if (player.hasTag("admin") == true, eventData.itemStack.typeId == "minecraft:wooden_pickaxe") {
            ...
            paintBlocksSplit.forEach(item => percentMsgReplace[percentMsgReplace.length] = Math.floor(item.split("%")[0]));
            paintBlocksSplit.forEach(item => blockMsgReplace[blockMsgReplace.length] = item.split("%")[1]);
